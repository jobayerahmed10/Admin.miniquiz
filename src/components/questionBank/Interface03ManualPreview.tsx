...
export const Interface03ManualPreview: React.FC<Interface03ManualPreviewProps> = ({
  questions,
  existingQuestions,
  meta,
  onBackToStep1,
  ...
  const [workingList, setWorkingList] = useState<WorkingQuestion[]>(questions);
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200 pb-12">
      ...
      {/* 3. Preview Summary Stats Card (Screenshot 3) */}
      <div className="bg-[#0b1322] border border-slate-800/90 rounded-3xl p-5 sm:p-6 shadow-xl space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-sm font-black text-white">
              প্রিভিউ (মোট {checkedQuestions.length} টি প্রশ্ন)
            </h2>
            <p className="text-xs text-slate-400 mt-0.5">
              প্রকাশ করার আগে প্রশ্নগুলো সঠিকভাবে যাচাই করুন
            </p>
          </div>
        </div>
              ...
              <div className="flex items-start justify-between gap-3 border-b border-slate-800/80 pb-3">
                <div className="space-y-1.5 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {q.isDuplicate ? (
                      <span className="px-2 py-0.5 rounded-md bg-rose-500/20 text-rose-300 text-[10px] font-bold border border-rose-500/40 flex items-center gap-1">
      ...
      {/* 5. Duplicate and Quality Validation Checklist Card (Screenshot 3) */}
      ...
      {/* 6. Big Rocket Publish Button (Screenshot 3) */}
