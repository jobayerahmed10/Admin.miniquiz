...
export const PracticeSection: React.FC<PracticeSectionProps> = ({
  ...
  const hierarchyData = useMemo(() => {
    // Group all questions by subject
  ...
  const startPractice = (
    ...
    let practiceQs = questions.filter((q) => qIds.has(q.id));
    if (practiceQs.length === 0) {
      // Fallback: match by title string if Set was empty
      practiceQs = questions.filter((q) => {
        if (type === 'subject') return isSameSubject(q.subject, title);
        if (type === 'topic') return (q.topic || '').trim().toLowerCase() === title.trim().toLowerCase();
        if (type === 'subtopic')
          return (
            (q.sub_topic || '').trim().toLowerCase() === title.trim().toLowerCase() ||
            (q.topic || '').trim().toLowerCase() === title.trim().toLowerCase()
          );
        return false;
      });
    }
